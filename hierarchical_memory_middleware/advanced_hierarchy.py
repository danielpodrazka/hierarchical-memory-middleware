...
class HierarchyThresholds:
    """Thresholds for different compression levels."""
    ...
    meta_group_size: int = 20    # Nodes per META group
...
class AdvancedHierarchyManager:
    ...
    def compress_to_summary(
        ...
        for node in nodes:
            ...
            result = CompressionResult(
                ...
                compression_ratio=len(enhanced_summary) / len(node.content) if node.content else 1.0,
    ...
    def compress_to_meta(
        ...
        for node in sorted_nodes:
            ...
            if (len(current_group) >= self.thresholds.meta_group_size and 
                len(current_group) <= self.thresholds.meta_group_max):
    ...
    def compress_to_archive(
        ...
        for node in nodes:
            ...
            result = CompressionResult(
                ...
                compression_ratio=len(archive_summary) / len(node.content) if node.content else 1.0,
    ...
    def _create_enhanced_summary(self, node: ConversationNode) -> str:
        ...
        base_summary = first_sentence if len(first_sentence) <= len(first_50_words) else first_50_words
            ...
            if CompressionLevel.SUMMARY in compression_needs:
                ...
                for result in summary_results:
                    await storage.compress_node(
                        ...
                        conversation_id=nodes[0].conversation_id,  # Assume same conversation
