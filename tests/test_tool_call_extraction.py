...
class TestToolCallExtraction:
    ...
    async def manager(self, tmp_path):
        ...
        config = Config(
            ...
            work_model="test:test-model",  # Use a test model that won't require API key
            log_tool_calls=True
        )
        storage = DuckDBStorage(config.db_path)
        
        # Mock the Agent to avoid API key requirement
        with patch('hierarchical_memory_middleware.middleware.conversation_manager.Agent') as mock_agent_class:
    ...
    async def test_log_tool_call_captures_calls_and_results(self, manager):
        ...
        mock_call_tool = AsyncMock(return_value={"status": "success", "data": "test result"})
        ...
        assert tool_result['content'] == str({"status": "success", "data": "test result"})
    ...
    async def test_multiple_tool_calls(self, manager):
        ...
        for i, (tool_name, args) in enumerate(tools):
            ...
            assert manager._current_tool_results[i]['content'] == str({"result": f"{['first', 'second', 'third'][i]} result"})
