...
Turn any Slack channel into an AI-powered workspace with persistent memory. The bot maintains context across conversations - mention it in any channel and it remembers previous discussions.

```bash
# Install and run
hmm-slack /path/to/your/project
...
2. Enable Socket Mode and add bot scopes (`app_mentions:read`, `channels:history`, `chat:write`)
3. Set `SLACK_BOT_TOKEN` and `SLACK_APP_TOKEN` environment variables
4. Run `hmm-slack /path/to/project`

See [Slack Bot Documentation](docs/slack-bot-setup.md) for detailed setup instructions.
